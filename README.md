...
| GET    | /users/:userId              |                                                     | json(response.data)                    | Retrieve  user data. Events, friends, lists of the user can be populated |
| POST   | /users/:userId/edit         |const userId = req.params.userId; const {picture,email} = req.body | json(response.data)                    | Update user information (picture, email).                                        |
